...
class MLFlowOps():
    def __init__(self, runs_names: list, base_model_name: str, experiment_ids: str = None, sorting_metric: str = 'metrics.val_rmse', ascending: bool = True):
    ...
    def get_best_run_model(self, run_name: str):
        mlflow_runs = mlflow.search_runs(experiment_ids=self.experiment_ids, filter_string=f"run_name='{run_name}'")
        sorted_runs = mlflow_runs.sort_values(by=[self.sorting_metric], ascending=self.ascending)
    ...
    def check_model_improvement_and_transition_in_training_loop(self, training_loss, training_run_id, stage = "Production"):
        ...
        if self.has_improved(run_name = run_name, most_recent_loss = training_loss):
            model_version = self.register_best_model(run_id = training_run_id, model_name = model_name)
    ...
    def register_and_transition_models(self, stage = "Production"):
        for run_name in self.runs_names:
            model_name =  f"{self.base_model_name}_{run_name}"
            most_recent_run = self.get_most_recent_run_model(run_name) #self.get_best_run_model(run_name)['run_id']
            if self.has_improved(run_name = run_name, most_recent_loss = most_recent_run[self.sorting_metric]):
                model_version = self.register_best_model(run_id = most_recent_run['run_id'], model_name = model_name)
                transitioned_model = self.transition_model(model_name, model_version, stage)
                return transitioned_model
    ...
    def list_registered_models(self):
        return [(model.name, model.latest_versions) for model in self.get_registered_models()]
    
    def delete_experiment_runs(self, state = "FAILED"):
        # Get a list of all runs in the experiment
        runs = mlflow.search_runs(self.experiment_ids, filter_string=f"status='{state}'")
    ...
    def purge_registered_models(self):
        while len(self.get_registered_models()) > 0:
            for model in self.get_registered_models():
                for version in model.latest_versions:
                    if version.current_stage in ["Staging", "Production"]:
                        print(f'Transitioning {model.name} and version {version.version} to None')
                        mlflow.tracking.MlflowClient().transition_model_version_stage(
                                name=model.name,
                                version=version.version,
                                stage="None"
